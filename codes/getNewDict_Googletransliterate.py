...
def main():
	...
	for i in range(0,len(glos)):
		...
		#print(words)
		if len(words)>1:
			map_words = transliteration(word[0],'text',output_lang_tag)
		else:
			map_words = transliteration(word[0],'word',output_lang_tag)
		#print(map_words)
		if isinstance(map_words, list):
			map_word = ''.join(map_words[0])
		else:
			map_word = map_words
		fid.write(word[0]+'\t'+map_word+'\n')
