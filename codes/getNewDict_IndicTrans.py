...
def main():
	...
	# hin, guj, pan, ben, mal, kan, tam, tel, ori, eng, mar, nep, bod, kok, asm, urd

	input_lang_tag = sys.argv[3]
	output_lang_tag = sys.argv[4]
	trans = Transliterator(source=input_lang_tag, target=output_lang_tag, build_lookup=True);
	...
	for i in range(0,len(glos)):
		word = glos[i].split('\t')
		fid.write(word[0]+'\t'+trans.transform(word[1]))
