...
def ex_cmd_print():
	ex_cmd_str='\nExample: python3 getDictFromSubFile_v1.py -i <input-file> -s <input-sub-file> -o <output-file>\n'
	return ex_cmd_str
			...
			if(also_has(sub_data[i],'en') or also_has(sub_data[i],'num')):
				...
				if (len(inp_lines) == len(sub_lines)):
					...
					for j in range(0,len(sub_lines)):
						...
						print(sub_lines)
						print(inp_lines)
						inp_line = inp_lines[k+inp_idx_plus].strip()
						sub_line = sub_lines[j].strip()
						if(also_has(sub_line,'en') or also_has(sub_line,'num')):
							flag = 1
							glos_word.append(sub_line)
							map_word.append(inp_line)
						else:
							if flag == 1:
								if (sub_line == inp_line):
									map_words = ' '.join(map_word)
									glos_words =  ' '.join(glos_word)
									if glos_words in glos.keys():
										if not (map_words in glos[glos_words]):
											glos[glos_words].append(map_words)
									else:
										glos[glos_words] = [map_words];
									glos_word = []
									map_word = []
									flag = 0
								else:
									while (sub_line != inp_line and j < len(sub_lines) and j+inp_idx_plus < len(inp_lines)):
										print(glos_word)
										print(map_word)
										map_word.append(inp_line)
										inp_idx_plus = inp_idx_plus + 1
										inp_line = inp_lines[j+inp_idx_plus].strip()
									map_words = ' '.join(map_word)
									glos_words =  ' '.join(glos_word)
									if glos_words in glos.keys():
										if not (map_words in glos[glos_words]):
